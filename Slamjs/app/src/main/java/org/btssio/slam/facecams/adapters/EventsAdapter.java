...
public class EventsAdapter extends ArrayAdapter<Events> {
	...
	private Events objBean;
	private int row;

	public EventsAdapter(Activity act, int resource, List<Events> arrayList) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (holder.place != null && null != objBean.getPlace()
				&& objBean.getPlace().trim().length() > 0) {
			holder.place.setText(Html.fromHtml(objBean.getPlace()));
		}
	...
	public class ViewHolder {
		public TextView nom, type, date, place;
	}
}